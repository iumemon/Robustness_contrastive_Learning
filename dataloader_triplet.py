...
class TrainDataset(torch.utils.data.Dataset):
    ...
    def get_negative_img(self, img_name):
        ...
        char_no = name.split('_')[1]
        # Get random character
        char = random.randint(1,704)
        while char == char_no:
            char = random.randint(1,704)
        style_img_name = str(char)+"_" + str(char_no)+file_extension
    ...
    def __getitem__(self, index):
        ...
        negative_image_name = self.get_negative_img(self.filenames[index])
        positive_image_path = self.get_image_path(negative_image_name)
        negative_image = io.imread(positive_image_path)
        ...
        x3 = self.tensorify(self.augmented_image(Image.fromarray(negative_image)))


        y_label = torch.tensor(int(c_label))
        
        return x1,x2,x3, y_label
...
if __name__ == '__main__':
    ...
    def show():
        ...
        images1, images2, images3, labels = next(iter(dataloaders['train']))
        fig, axs = plt.subplots(1, 3)
        axs[0].imshow(images1[0][0], cmap = 'gray')
        axs[1].imshow(images2[0][0], cmap = 'gray')
        axs[2].imshow(images3[0][0], cmap = 'gray')
